WITH tb_usuario_cursos AS (
    SELECT
        idUsuario,
        descSlugCurso,
        COUNT(descSlugCursoEpisodio)  qtdeEp
    FROM cursos_episodios_completos
    WHERE dtCriacao < '2026-02-04'
...
tb_pct_cursos AS (
    SELECT
        t1.idUsuario,
        t1.descSlugCurso,
        1. * t1.qtdeEp / t2.qtdeTotalEps as pctCursoCompleto
...
tb_pct_cursos_pivot AS (
    SELECT
        idUsuario,
        SUM(CASE WHEN pctCursoCompleto = 1 THEN 1 ELSE 0 END) qtdeCursosCompletos,
        SUM(CASE WHEN pctCursoCompleto > 0 AND pctCursoCompleto < 1 THEN 1 ELSE 0 END) qtdeCursosIncompletos,
...
tb_ultima_atividade AS (
    SELECT
        idUsuario,
        MIN(julianday(date('2026-02-04')) - julianday(dtCriacao)) AS qtdeDiasUltiAtividade
    FROM tb_atividade
    GROUP BY idUsuario
),
...
SELECT
    date('2026-02-04', '-1 day') AS dtRef,
    t1.*
FROM tb_join AS t1